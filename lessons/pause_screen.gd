...
@export_range(0, 1) var menu_opened_amount := 0:
...
func set_menu_opened_amount(amount: float) -> void:
	...
	if _ui_panel_container == null or _blur_color_rect == null:
		return
	_blur_color_rect.material.set_shader_parameter("blur_amount", lerp(0, 1.5, amount))
	_blur_color_rect.material.set_shader_parameter("saturation", lerp(1, 0.3, amount))
